...
export const decompressCommandHandler = async ({args: [srcPath, destPath], fileManager: { currentLocaction }}) => {
  try {
    ...
    await stream.pipeline(readStream, zlib.createBrotliDecompress(), writeStream);
  } catch (error) {
    throw new OperationFailed();
  }
};